...
int main() {
  ...
  while (cellsToVisit.size() != 0) {
    ...
    if (coordsEqual(startCoords, currentCell.getCoords())) {
      currentCell.distance = 0;  // first step has distance of 0
    }
    ...
    for (int i = 0; i < neighbors.size(); i++) {
      ...
      if (!nextCell.visited || nextCell.distance > currentCell.distance + 1) {
        ...
        if (coordsEqual(startCoords, grid[21][0].getCoords())) {
          std::cout << "some idiot called the wrong cell" << std::endl;
        }
